...
resource "proxmox_virtual_environment_file" "cloud_config-alfamart" {
  ...
  source_raw {
    ...
    runcmd:
      ...
      - ip link set eth1 up 
      - ip link set eth2 up 
      - ip route add 0.0.0.0/0 via 192.168.5.6 #ALFAMARTviaINDOMARET
...
resource "proxmox_virtual_environment_file" "cloud_config-indomaret" {
  ...
  source_raw {
    ...
    runcmd:
      ...
      - ip link set eth0 up 
      - ip link set eth1 up 
      - ip route add 0.0.0.0/0 via 192.168.5.10 #INDOMARETviaFAMILYMART
      - ip route add 192.168.5.24/29 via 192.168.5.5 #INDOMARETviaALFAMARTA1
      - ip route add 192.168.5.0/30 via 192.168.5.5 #INDOMARETviaALFAMARTA2
      - echo "Initialization complete" > /tmp/init_done
    EOF
...
resource "proxmox_virtual_environment_file" "cloud_config-family-mart" {
  ...
  source_raw {
    ...
    runcmd:
      ...
      - ip link set eth1 up
      - ip link set eth2 up
      - ip route add 192.168.5.0/30 via 192.168.5.9 
      - ip route add 192.168.5.4/30 via 192.168.5.9 
      - ip route add 192.168.5.24/29 via 192.168.5.9 #FAMILYMARTviaINDOMARETA1
      - ip route add 192.168.5.16/30 via 192.168.5.14 #FAMILYMARTviaSUPERINDOA6
      - ip route add 192.168.5.32/29 via 192.168.5.14 #FAMILYMARTviaSUPERINDOA7
      - ip route add 192.168.5.40/29 via 192.168.5.14 #FAMILYMARTviaSUPERINDOA8
      - ip route add 192.168.5.48/29 via 192.168.5.14 #FAMILYMARTviaSUPERINDOA9
      - echo "Initialization complete" > /tmp/init_done
    EOF
...
resource "proxmox_virtual_environment_file" "cloud_config-its-mart" {
  ...
  source_raw {
    ...
    runcmd:
      ...
      - ip link set eth0 up
      - ip link set eth1 up
      - ip route add 0.0.0.0/0 via 192.168.5.41 #ITSMARTviaSUPERINDO
      - echo "Initialization complete" > /tmp/init_done
    EOF
...
resource "proxmox_virtual_environment_file" "cloud_config-superindo" {
  ...
  source_raw {
    ...
    runcmd:
      ...
      - ip link set eth1 up 
      - ip link set eth2 up 
      - ip route add 0.0.0.0/0 via 192.168.5.13 #SUPERINDOviaFAMILYMART
      - ip route add 192.168.5.32/29 via 192.168.5.18 #SUPERINDOviaSAKINAHA7
      - ip route add 192.168.5.48/29 via 192.168.5.43 #SUPERINDOviaITSMARTA9
      - echo "Initialization complete" > /tmp/init_done
    EOF
...
resource "proxmox_virtual_environment_file" "cloud_config-sakinah" {
  ...
  source_raw {
    ...
    runcmd:
      ...
      - ip link set eth0 up
      - ip link set eth1 up
      - ip route add 0.0.0.0/0 via 192.168.5.17 #SAKINAHviaSUPERINDO
      - echo "Initialization complete" > /tmp/init_done
    EOF
...
resource "proxmox_virtual_environment_vm" "family-mart-D05" {
  ...
  initialization {
    ...
    ip_config { #eth2
      ipv4 {
        address = lookup(var.ip_list, "family-mart-D05-eth2")
      }
    }
    # user_account {
    #   username = "ukashitam"
